...
const EnhancedAIFallbackTester = () => {
  const [testResults, setTestResults] = useState<TestResult[]>([]);
  const [isRunning, setIsRunning] = useState(false);
  const [progress, setProgress] = useState(0);
  const [selectedService, setSelectedService] = useState("all");

  // Enhanced test configurations with detailed monitoring
  const testSuites = {
    payment: [
      {
        id: "payment-init",
  ...
  const testWithDetailedStatus = async (functionName: string, payload: any) => {
    ...
    try {
      // Try to manually test each layer to get detailed status
      let result: any = null;
        ...
        try {
          ...
          if (vercelHealthy) {
            ...
            result = {
              ...
              data: {
                fallback: true,
                message: "Vercel disabled, using fallback",
                originalPayload: payload,
              },
              source: "fallback",
              timestamp: Date.now(),
        ...
        if (vercelHealthy) {
          try {
            ...
            result = {
              ...
              data: {
                fallback: true,
                message: "Both primary services failed",
                originalPayload: payload,
              },
              source: "fallback",
              timestamp: Date.now(),
          ...
          layerStatus.vercel = "skipped";
          executionPath.push("Fallback (All services disabled)");
          layerStatus.fallback = "success";
          result = {
            success: true,
            data: {
              fallback: true,
              message: "All services disabled, using fallback only",
              originalPayload: payload,
            },
            source: "fallback",
            timestamp: Date.now(),
      ...
      result.layerStatus = layerStatus;
  ...
  const generateDetailedMessage = (result: any): string => {
    ...
    const { layerStatus, executionPath, source } = result;
    const parts: string[] = [];
    ...
    const sourceText = source ? ` (Final source: ${source.toUpperCase()})` : "";
    ...
    for (let i = 0; i < allTests.length; i++) {
      ...
      try {
        ...
        const testResult: TestResult = {
          ...
          detailedMessage: generateDetailedMessage(result),
          duration,
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Enhanced AI Fallback System Tester
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="text-center text-muted-foreground">
                No test results yet. Click "Run Tests" to start testing with
...
export default EnhancedAIFallbackTester;
