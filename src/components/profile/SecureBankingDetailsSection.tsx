...
const SecureBankingDetailsSection: React.FC = () => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    checkForBankingDetails();
  }, [user]);

  const checkForBankingDetails = async () => {
    if (!user?.id) return;

    try {
      setIsLoading(true);
      const hasDetails = await SecureBankingService.hasBankingDetails(user.id);
      if (hasDetails) {
  ...
  const loadBankingDetails = async () => {
    ...
    try {
      setIsLoading(true);
      const details = await SecureBankingService.getBankingDetails(
        user.id,
        password,
      );
      if (details) {
        ...
        setPassword("");
        toast.success("Banking details loaded successfully!");
      } else {
        toast.error(
          "Failed to load banking details. Please check your password.",
        );
      }
    } catch (error) {
      const errorMessage =
  ...
  const handleSaveWithPassword = async () => {
    ...
    try {
      ...
      const savedDetails = await SecureBankingService.saveBankingDetails(
        detailsToSave,
        password,
      );
      ...
      setNeedsPassword(false);
      setPassword("");
      toast.success("Banking details saved and encrypted successfully!");
    } catch (error) {
  ...
  const handleDeleteWithPassword = async () => {
    ...
    try {
      setIsLoading(true);
      await SecureBankingService.deleteBankingDetails(user.id, password);
  ...
  if (isLoading && !bankingDetails && !needsPassword) {
    return (
      <Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Enter Password to Access Banking Details
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <Alert>
            ...
            <AlertDescription>
              Your banking details are encrypted with your password for maximum
              security.
              {passwordAction === "load" &&
                " Enter your password to view your details."}
              {passwordAction === "save" &&
                " Enter your password to encrypt and save your banking details."}
          ...
          <Alert variant="destructive">
            ...
            <AlertDescription>
              <strong>Important:</strong> This is your login password. Your
              banking details are encrypted using this password. If you forget
              your password, your banking details cannot be recovered.
            </AlertDescription>
          </Alert>
        </CardContent>
      </Card>
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <Alert>
          <Shield className="h-4 w-4" />
          <AlertDescription>
            Your banking information is encrypted with your password and stored
            securely. This is required to receive payments from book sales.
          </AlertDescription>
        </Alert>
          ...
          <div className="space-y-4">
            <div className="bg-green-50 p-3 rounded-lg border border-green-200">
              <div className="flex items-center gap-2 text-green-800">
                ...
                <span className="font-medium">Details Encrypted & Secure</span>
              </div>
            </div>
        ...
        {isEditing && (
          <div className="space-y-4">
            ...
            <Alert>
              <Lock className="h-4 w-4" />
              <AlertDescription>
                Your banking details will be encrypted with your login password
                before being saved.
              </AlertDescription>
            </Alert>
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                {isLoading ? "Saving..." : "Save & Encrypt Details"}
              </Button>
              <Button
                variant="outline"
...
export default SecureBankingDetailsSection;
