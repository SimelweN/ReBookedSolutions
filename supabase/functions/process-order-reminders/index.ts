...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: ordersNeedingReminders, error: reminderError } =
      await supabase
        .from("orders")
        .select(
          `
    ...
    const { data: ordersNeedingCollectionReminders, error: collectionError } =
      await supabase
        .from("orders")
        .select(
          `
    ...
    for (const order of ordersNeedingCollectionReminders || []) {
      try {
        ...
        console.error(
          `Error sending collection reminder for order ${order.id}:`,
          error,
        );
    ...
    await supabase.from("audit_logs").insert({
      ...
      new_values: {
        ...
        totalOrdersProcessed:
          (ordersNeedingReminders?.length || 0) +
          (ordersNeedingCollectionReminders?.length || 0),
      },
    });
